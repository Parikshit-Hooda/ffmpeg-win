...
var ffmpeg = __dirname + '/ffmpeg.exe';
var ffprobe = require('ffprobe-win');
...
function encode(ext, src, dest, cb) {
  ...
  if (!exists(src)) {
    cb(new Error('ffmpeg-win: source not found'));
    return;
  }
  ...
  getduration(src, function(err, duration) {
    if (err) cb(err);
      ...
      child.on('close', function(code) {
        bar.update(1);
        if (code != 0) var err = new Error('ffmpeg-win: encoding error');
        cb(err || null);
      });
    }
  });
